...
// `flowchart LR|TD` 한 줄, `subgraph <id>["<이름>"]`(중첩)·`end`, `<id>["<이름>"]`, `<a> --> <b>` 만 읽고 그 밖의 문법(라벨 화살표·점선·classDef·style 등)은
// 경고 없이 무시하고 건수만 남긴다. 첫 줄을 못 찾으면 { ok: false, reason: 'no-flowchart' }(architecture.diagram-unreadable 의 근거).
...
const SUBGRAPH = new RegExp(`^\\s*subgraph\\s+(${ID})(?:\\s*\\[\\s*"?([^"\\]]*?)"?\\s*\\])?\\s*$`);
const END = /^\s*end\s*$/;
const NODE = new RegExp(`^\\s*(${ID})\\s*\\[\\s*"?([^"\\]]*?)"?\\s*\\]\\s*$`);
const BARE = new RegExp(`^\\s*(${ID})\\s*$`);
const EDGE = new RegExp(`^\\s*(${ID})\\s*-->\\s*(${ID})\\s*$`);
...
export function parseFlowchart(text) {
  ...
  for (const line of lines.slice(1)) {
    ...
    if ((m = line.match(EDGE))) {
      node(m[1], null, stack.at(-1) ?? null); node(m[2], null, stack.at(-1) ?? null);
      const k = `${m[1]}\u0000${m[2]}`;
      if (!edgeKeys.has(k)) { edgeKeys.add(k); edges.push({ from: m[1], to: m[2] }); }
      continue;
    }
