...
- **커밋 전 훅의 끝 줄이 구조 문제를 막고도 작업 문서 얘기만 했다.** `check --staged`는 2.1.0부터 갈래 둘을 막는데(작업 문서·판정 파일의 `tasks.*`·`judgment.*`, 선언이 스테이징됐을 때 그 선언에서 고칠 수 있는 `architecture.*`), 끝 줄은 늘 "스테이징된 작업 문서·판정 파일의 tasks.*·judgment.* 문제"라고 말하고 `map/judgments/<작업 폴더>.json`에 판정을 적으라고 안내했다. 층 위반으로 커밋이 막힌 사람에게 그 자리에 없는 처리를 가리킨 것이다. 이제 끝 줄은 실제로 센 문제의 종류를 따라, 구조 문제면 `map/architecture/`의 선언과 `map/config.json`을, 작업 문서 문제면 원문과 판정 파일을, 둘 다면 둘 다 가리킨다. 같은 어긋남을 `livemap init`이 쓰는 훅 주석, `--help`, `README.md`, `docs/issue-codes.md`에서도 고쳤다. 검사: `test/staged-hook.test.mjs` SC-13d.
...
- 클릭 대상 크롤러(`scripts/route-crawl.mjs`)가 이동 뒤 그 해시의 화면이 서고 「불러오는 중…」이 사라진 다음 잰다. 그전에는 DOM이 200ms 잠잠하면 바로 쟀는데, 개요에서 하위 화면으로 가면 화면이 `data.json`을 받는 동안 `[data-screen]`이 없어서 그 사이에 재면 화면 없음·개체 없음으로 틀렸다. 재빌드 `serve`는 자료 요청이 마지막 빌드 5초 뒤면 빌드를 먼저 해서 응답이 수 초(실측 4.8초) 늦고, 그래서 `--only roadmap`이 특정 행에서, 전체 크롤이 간헐적으로 실패했다. 회귀 검사(`test/route-crawl.test.mjs`)가 `data.json` 응답을 늦춰 이 조건을 고정한다. `docs/view-budget.md`에서 「`--only roadmap`은 원인 미확정으로 실패한다」는 문장을 지웠다. 팩에 든 코드는 바뀌지 않는다.
