...
test('SC-19 NAV_ITEMS 는 화면 NAV 길이와 같다(Phase 3 이 NAV 를 늘리면 상수도 함께 올린다)', () => {
  assert.equal(NAV_ITEMS, NAV.length);
  assert.equal(NAV_ITEMS, 5);
});
...
test('SC-19 budget.navItems 가 NAV_ITEMS 보다 작으면 budget.nav-items-low 한 줄, 같거나 크거나 없으면 없음', () => {
  ...
  const low = checkProblems(d, { budget: { navItems: 4 } }).filter((p) => p.code === 'budget.nav-items-low');
  assert.equal(low.length, 1);
  assert.deepEqual([low[0].level, low[0].subject, low[0].resolutions], ['warn', { kind: 'config', id: 'budget.navItems' }, ['config']]);
  assert.match(low[0].msg, /4.*5/);
  assert.equal(checkProblems(d, { budget: { navItems: 5 } }).filter((p) => p.code === 'budget.nav-items-low').length, 0);
...
test('SC-19 명령: mini 사본의 navItems 를 4 로 두면 check 에 budget.nav-items-low 한 줄이고 --strict 에서도 경고(종료 코드는 기존 오류 때문에 1)', () => {
