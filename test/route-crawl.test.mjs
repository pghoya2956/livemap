...
test.before(async () => {
  ...
  // 필수 셋과 있으면 쓰는 것(architecture.json·md)까지 옮긴다. 수준 토글이 architecture.json 을 받는다
  for (const f of DATA_FILES) cpSync(join(out, f), join(siteDir, 'data', f));
  for (const f of OPTIONAL_DATA_FILES) if (existsSync(join(out, f))) cpSync(join(out, f), join(siteDir, 'data', f));
