...
처음 보는 동료·외부 독자가 코드를 읽지 않고 "지금 어느 마일스톤이고 얼마나 왔나, 무엇이 동작하고 무엇이 남았나, 무엇을 기다리나, 최근 얼마나 바뀌었나"를 한 화면에서 보고, 기능 → 단계 → 화면·API·DB·검사·결정·작업으로 파고드는 뷰다. 엔진은 npm 패키지 `@pghoya2956/livemap`이고, 이 폴더에는 프로젝트 소유 파일만 둔다. 배포본 주소: `<배포 주소>/map/`. 실시간은 로컬 `npm run map:serve`.
